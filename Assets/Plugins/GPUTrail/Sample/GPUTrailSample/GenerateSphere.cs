...
public class GenerateSphere : MonoBehaviour {
	...
	void Awake () {
        ...
        for(var i=0; i<num; ++i)
        {
            ...
            if (go)
            {
                ...
                go.GetComponent<Rigidbody>().linearVelocity = Random.insideUnitSphere * 10f;
            }
        }
    ...
    void Update()
    {
        if ( Input.GetMouseButton(0))
        {
            ...
            {
                go.transform.SetParent(transform);
                go.GetComponent<Rigidbody>().linearVelocity = Camera.main.transform.TransformVector(Quaternion.Euler(Random.Range(-30f, 10f), 0f, 0f) * Vector3.forward * Random.Range(10f, 100f));
            }
        }
    }

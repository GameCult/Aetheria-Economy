...
float HierarchicalRaymarch(float3 startPos, float3 dir, float maxSampleDistance, float raymarchOffset, out float intensity, out float depth, out int iteration_count) {
	...
	[loop]
    while(currentStep < maxStepCount && iteration_count++ < SAMPLE_COUNT) {

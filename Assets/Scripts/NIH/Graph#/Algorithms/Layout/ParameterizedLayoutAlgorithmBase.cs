...
namespace GraphSharp.Algorithms.Layout
{
		...
		where TParam : class, ILayoutParameters
	{
		...
		protected ParameterizedLayoutAlgorithmBase( TGraph visitedGraph, IDictionary<TVertex, float2> vertexPositions,
		...
		protected override ILayoutIterationEventArgs<TVertex> CreateLayoutIterationEventArgs( int iteration, float statusInPercent, string message, IDictionary<TVertex, float2> vertexPositions )
		{
        ...
        where TParam : class, ILayoutParameters, new()
    {
        ...
        protected DefaultParameterizedLayoutAlgorithmBase(TGraph visitedGraph, IDictionary<TVertex, float2> vertexPositions, TParam oldParameters) 
            : base(visitedGraph, vertexPositions, oldParameters)
        {
        }
		...
		where TParam : class, ILayoutParameters
	{
		...
		/// 
		/// Bounding box:
		/// x coordinates: float.Epsilon - <code>width</code>
		/// y coordinates: float.Epsilon - <code>height</code>
		/// </summary>
		/// <param name="width">Width of the bounding box.</param>
		/// <param name="height">Height of the bounding box.</param>
		protected virtual void InitializeWithRandomPositions( float width, float height )
		{
		...
		protected virtual void InitializeWithRandomPositions( float width, float height, float translate_x, float translate_y )
		{
			...
			foreach ( TVertex v in VisitedGraph.Vertices )
			{
				...
				if ( !VertexPositions.ContainsKey( v ) )
				{
					VertexPositions[v] =
						new float2(
							max( float.Epsilon, (float) rnd.NextDouble() * width + translate_x ),
							max( float.Epsilon, (float) rnd.NextDouble() * height + translate_y ) );
				}
			}
		}
		...
		protected static void NormalizePositions( IDictionary<TVertex, float2> vertexPositions )
		{
			if ( vertexPositions == null || vertexPositions.Count == 0 )
				return;

			//get the topLeft position
			var topLeft = new float2( float.PositiveInfinity, float.PositiveInfinity );
		...
		protected void OnIterationEnded( int iteration, float statusInPercent, string message, bool normalizePositions )
		{
			//copy the actual positions
			IDictionary<TVertex, float2> vertexPositions;
			lock ( SyncRoot )
			{
				vertexPositions = new Dictionary<TVertex, float2>( VertexPositions );
			}
		...
		protected virtual ILayoutIterationEventArgs<TVertex> CreateLayoutIterationEventArgs( int iteration, float statusInPercent, string message, IDictionary<TVertex, float2> vertexPositions )
		{

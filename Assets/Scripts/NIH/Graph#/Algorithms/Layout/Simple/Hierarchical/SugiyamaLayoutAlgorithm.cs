...
namespace GraphSharp.Algorithms.Layout.Simple.Hierarchical
{
        ...
        where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
    {
        ...
        */
        ...
        public IDictionary<TEdge, float2[]> EdgeRoutes { get; private set; }
            ...
            : base( visitedGraph, vertexPositions, parameters )
        {
            ...
            EdgeRoutes = new Dictionary<TEdge, float2[]>();
        ...
        /// </summary>
        protected void ConvertGraph( IDictionary<TVertex, float2> vertexSizes )
        {
            ...
            foreach ( TVertex v in VisitedGraph.Vertices )
            {
                ...
                size.y += Parameters.VerticalGap;
                size.x += Parameters.HorizontalGap;
        ...
        protected void ReplaceLongEdges()
        {
            ...
            foreach ( var edge in _graph.HierarchicalEdges.ToArray() )
            {
                ...
                for ( int i = sourceLayerIndex + 1; i <= targetLayerIndex; i++ )
                {
                    ...
                    {
                        dummy = new SugiVertex( null, new float2( 0, 0 ) );
        ...
        protected void SugiyamaLayout()
        {
            ...
            float maxIterations = iteration1Left * iteration2Left;
        ...
        private float CalculateOverlap( SugiVertex a, SugiVertex b, float plusGap )
        {
            return Math.Max( 0, ( ( b.Size.x + a.Size.x ) * 0.5f + plusGap + Parameters.HorizontalGap ) - ( b.RealPosition.x - a.RealPosition.x ) );
        }
        ...
        protected void HorizontalPositionAssignmentOnLayer( int layerIndex, BaryCenter baryCenter )
        {
                               ...
                               select vertex )
            {
                ...
                SugiVertex v1 = v;
                var alreadySetVertices = layer.Where( vertex => ( !float.IsNaN( vertex.RealPosition.x ) && vertex != v1 ) ).ToArray();
                ...
                float leftOverlap = 0, rightOverlap = 0;
                ...
                if ( leftOverlap > 0 && rightOverlap == 0 )
                {
                    ...
                    {
                        float leftMove = leftOverlap * 0.5f;
                        ...
                        if ( rightOverlap > 0 )
                        {
                            ...
                            {
                                float rightMove = rightOverlap * 0.5f;
        ...
        protected void AssignPositions()
        {
            ...
            {
                float pos = 0;
                float layerHeight = _layers[i].Height;
                foreach ( var v in _layers[i] )
                {
                    v.RealPosition.x = pos;
                    v.RealPosition.y =
                        ( ( i == 0 )
                            ? ( layerHeight - v.Size.y )
                            : verticalPos + layerHeight * (float)0.5 );

                    pos += v.Size.x + Parameters.HorizontalGap;
                }
        ...
        protected void CopyPositionsSilent( bool shouldTranslate )
        {
            ...
            var translation = new float2( float.PositiveInfinity, float.PositiveInfinity );
            if ( shouldTranslate )
            {
                foreach ( var v in _graph.Vertices )
                {
                    if ( float.IsNaN( v.RealPosition.x ) || float.IsNaN( v.RealPosition.y ) )
                        continue;
            ...
            foreach ( var v in _graph.Vertices )
            {
                ...
                float2 pos = v.RealPosition;
                if ( !shouldTranslate )
                {
                    pos.x += v.Size.x * 0.5f + translation.x;
                    pos.y += v.Size.y * 0.5f + translation.y;
                }

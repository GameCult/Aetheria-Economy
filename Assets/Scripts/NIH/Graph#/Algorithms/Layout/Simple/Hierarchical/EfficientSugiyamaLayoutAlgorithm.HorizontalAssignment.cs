...
namespace GraphSharp.Algorithms.Layout.Simple.Hierarchical
{
        ...
        where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
    {
        ...
        private float[] _layerHeights;
        private float[] _layerPositions;
        ...
        private void DoOrthogonalEdgeRouting()
        {
            ...
            {
                float2[] orthoRoutePoints = new float2[2];
                ...
                orthoRoutePoints[sourceIndex] = new float2()
                {
                    x = sourceVertex.HorizontalPosition,
                    y = _layerPositions[sourceVertex.LayerIndex] + _layerHeights[sourceVertex.LayerIndex] + Parameters.LayerDistance / 2.0f
                };
                orthoRoutePoints[targetIndex] = new float2()
                {
                    x = targetVertex.HorizontalPosition,
                    y = _layerPositions[targetVertex.LayerIndex] - Parameters.LayerDistance / 2.0f
                };
            ...
            {
                float2[] orthoRoutePoints = _edgeRoutingPoints[kvp.Key];

                var routePoints = new float2[kvp.Value.Count + 4];
                routePoints[0] = orthoRoutePoints[0];
                routePoints[kvp.Value.Count + 3] = orthoRoutePoints[1];
                for (int i = 0; i < kvp.Value.Count; i++)
                {
                    var vertex = kvp.Value[i];
                    routePoints[i+2] = new float2(vertex.HorizontalPosition, vertex.VerticalPosition);
                }
                routePoints[1] = new float2(routePoints[2].x, routePoints[0].y);
                routePoints[kvp.Value.Count + 2] = new float2(routePoints[kvp.Value.Count + 1].x, routePoints[kvp.Value.Count + 3].y);
        ...
        private void DoTraditionalEdgeRouting()
        {
            foreach (var kvp in _dummyVerticesOfEdges)
            {
                var routePoints = new float2[kvp.Value.Count];
                for (int i = 0; i < kvp.Value.Count; i++)
                {
                    var vertex = kvp.Value[i];
                    routePoints[i] = new float2(vertex.HorizontalPosition, vertex.VerticalPosition);
                }
                _edgeRoutingPoints[kvp.Key] = routePoints;
            }
        ...
        private void SavePositions()
        {
            foreach (var vertex in _graph.Vertices)
            {
                if (vertex.Type == VertexTypes.Original)
                    VertexPositions[vertex.OriginalVertex] = new float2(vertex.HorizontalPosition, vertex.VerticalPosition);
            }
        ...
        {
            _layerHeights = new float[_layers.Count];
            for (int i = 0; i < _layers.Count; i++)
                _layerHeights[i] = _layers[i].Max(v => v.Size.y);

            float layerDistance = Parameters.LayerDistance;
            _layerPositions = new float[_layers.Count];
            _layerPositions[0] = 0;
                ...
                if (Parameters.PositionMode < 0)
                {
                    vertex.HorizontalPosition =
                        (vertex.HorizontalPositions[0] + vertex.HorizontalPositions[1]
                         + vertex.HorizontalPositions[2] + vertex.HorizontalPositions[3]) / 4.0f;
                }
                else
                {
        ...
        {
            foreach (var vertex in _graph.Vertices)
                vertex.VerticalPosition = _layerPositions[vertex.LayerIndex] + (vertex.Size.y <= 0 ? _layerHeights[vertex.LayerIndex] : vertex.Size.y) / 2.0f;
        }
        ...
        private void InitializeSinksAndShifts(int modeIndex)
        {
            foreach (var vertex in _graph.Vertices)
            {
                ...
                vertex.Shifts[modeIndex] = float.PositiveInfinity;
                vertex.HorizontalPositions[modeIndex] = float.NaN;
            }
        }

        private void PlaceBlock(int modeIndex, LeftRightMode leftRightMode, UpperLowerEdges upperLowerEdges, SugiVertex v)
        {
            if (!float.IsNaN(v.HorizontalPositions[modeIndex]))
                return;

            float delta = Parameters.VertexDistance;
            ...
            do
            {
                      ...
                      || (leftRightMode == LeftRightMode.Right && _sparseCompactionGraph.OutDegree(w) > 0)))
                {
                    ...
                    foreach (var edge in edges)
                    {
                        ...
                        var xDelta = delta + ((wVertex?.Size.x ?? 0.0f) + ((pred is SugiVertex vertex) ? vertex.Size.x : u.BlockWidths[modeIndex])) / 2.0f;
        ...
        private void DoHorizontalCompaction(int modeIndex, LeftRightMode leftRightMode, UpperLowerEdges upperLowerEdges)
        {
            ...
            foreach (var vertex in _graph.Vertices)
            {
                ...
                if (vertex.Roots[modeIndex].Sinks[modeIndex].Shifts[modeIndex] < float.PositiveInfinity &&
                        ...
                        || (vertex.Type == VertexTypes.QVertex && upperLowerEdges == UpperLowerEdges.Lower))
                    {
                        ...
                        for (int m = 0; m < medians.Length; m++)
                        {
                                    ...
                                    || (leftRightMode == LeftRightMode.Right && r > neighbour.Position)))
                            {
                                ...
                                neighbour.BlockWidths[modeIndex] = max(neighbour.BlockWidths[modeIndex], vertex.Size.x);
        ...
        private void InitializeRootsAndAligns(int modeIndex)
        {
            foreach (var vertex in _graph.Vertices)
            {
                ...
                vertex.BlockWidths[modeIndex] = vertex.Size.x;
            }
        }
    }
}

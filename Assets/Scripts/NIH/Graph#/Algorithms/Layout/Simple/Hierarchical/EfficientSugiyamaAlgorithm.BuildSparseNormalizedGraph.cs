        ...
        where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
    {
        ...
        private bool DoWHOptimizationStep()
        {
            ...
            else
            {
                    ...
                    && insertedLayerInfo.LayerWidth <= (desiredWidth - _whOptLayerInfos[insertedLayerIndex - 1].Vertices.Peek().Cost))
                {
                    ...
                    height = Math.Max(height, repositionedVertex.Vertex.Size.y);
                }
        ...
        private void CreateLayerWHOptInfos()
        {
            ...
            foreach (var layer in _layers)
            {
                ...
                foreach (var vertex in layer)
                {
                    layerInfo.LayerHeight = Math.Max(vertex.Size.y, layerInfo.LayerHeight);
                    layerInfo.LayerWidth += vertex.Size.x;
        ...
        private void CreateVertexWHOptInfos()
        {
            ...
            foreach (var vertex in _graph.Vertices)
            {
                ...
                whOptInfo.Value = vertex.Size.x - Math.Max(0, _graph.InDegree(vertex) - 1) * Parameters.VertexDistance;
                whOptInfo.Cost = vertex.Size.x - Math.Max(0, _graph.OutDegree(vertex) - 1) * Parameters.VertexDistance;
                whOptInfo.Vertex = vertex;
                _whOptVertexInfos[vertex] = whOptInfo;
                vertexCount++;
                _whAverageVertexHeight += vertex.Size.y;
            }

...
InterpolatorsVertex MyVertexProgram (VertexData v) {
    ...
    float displacement = tex2Dlod(_DisplacementMap, float4(i.uv.xy, 0, 0)).r * _DisplacementStrength;
...
/*	float3 tangentSpaceNormal = GetTangentSpaceNormal(i);
	...
	i.normal = calcNormal(i.worldPos.xyz,i.uv.xy);
}
...
float cells( float3 p )
{
    ...
    //return -abs(1-(p.x+p.y+p.z));
    
    return min((p.x+p.y+p.z),length((p.x+p.y+p.z)-3.0));
...
FragmentOutput MyFragmentProgram (Interpolators i) {
    ...
    half inclination = 1 - saturate(dot (float3(0,1,0), i.normal));
	
    i.normal = dot(viewDir, i.normal) > 0 ? i.normal : -i.normal;
    
    half rim = saturate(dot (normalize(viewDir), i.normal));
    ...
    float3 dir = refract(viewDir,i.normal,_BackgroundFresnel);
	color.rgb += texCUBElod(_Background, float4(dir, _BackgroundBlur)).rgb * _Emission * pow(rim,_EmissionFresnel) * _EmissionColor;// * (max(sign(alpha - _AlphaClip),0)/2+.5);
